...
std::map<std::string, int> Analyzer::stlits;
...
Expr* Analyzer::factor()
{
  ...
  switch(curToken->tag){
    ...
    case STLIT:
      ...
      move();
      stlitCount++;
      std::cout << "str" << stlitCount << ": .asciiz " << "\"" << str->lexeme << "\"" << std::endl;
      Analyzer::stlits[str->lexeme] = stlitCount;
      return n;
  }
}

int Analyzer::getStLitNum(std::string st){
  return Analyzer::stlits[st];
}
...
bool Analyzer::is_number(const std::string& s)
{
